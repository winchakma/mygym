...
# Replace the widgetHTML
new_widget = """const widgetHTML = `
    <style>
        .ai-support-fab {
...
# Use regex to replace the widgetHTML string
pattern = r'const widgetHTML = `.*?`;'
content = re.sub(pattern, new_widget, content, flags=re.DOTALL)

# Add JS logic for the new functions and update existing ones
new_logic = """window.loadSupportHistory = async function() {
...
# Replace the functions logic
pattern_logic = r'window\.loadSupportHistory = async function\(\) \{.*\}'
